...
def extract_colors(image, n_colors=5):
    ...
    try:
        ...
        for i, color in enumerate(kmeans.cluster_centers_):
            label_count = np.sum(kmeans.labels_ == i)
            percentage = label_count / len(pixels) * 100
...
def get_color_name(hex_color):
    ...
    # Simple color name detection using RGB ranges
...
def calculate_compatibility(item1, item2):
    ...
    if item1.get('category') and item2.get('category'):
        tops = ['shirts', 't-shirts', 'dresses', 'sweater', 'hoodie']
