...
export const mlApi = {
  ...
  analyzeProduct: (url: string) => api.post('/ml/analyze-product', { url })
}

export const styleMatchApi = {
  analyze: (productUrl: string, gender?: string) =>
    api.post('/stylematch', { productUrl, gender }),
  getHistory: (params?: { page?: number; limit?: number }) =>
    api.get('/stylematch/history', { params })
}